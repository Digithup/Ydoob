...
class OrderView(LoginRequiredMixin, View):
    def get(self, request):
        if request.user.is_superuser:
            all_orders = order_models.Orders.objects.filter(
            ...
            my_delivery_object = delivery_utils.get_my_delivery_object(
                request.user)
        return render(request, template_version+"/Views/Order/list.html", context={"orders": all_orders})
...
class OrderDetail(LoginRequiredMixin, View):
    def get(self, request, id):
        ...
        try:
            order = order_models.Orders.objects.get(id=id)
        except (Exception, order_models.Orders.DoesNotExist):
        ...
        if flag:
            ...
            return render(request, template_version+"/Views/Order/detail.html", context=context)
        else:
            return HttpResponseRedirect(reverse('delivery-order'))
...
class MyDelivery(LoginRequiredMixin, View):
    def get(self, request):
        ...
        orders = order_models.Orders.objects.filter(Q(direct_assign=my_delivery_obj) | Q(delivery_by=my_delivery_obj) & ~Q(status=3) & ~Q(status=4)).order_by('created_at')
        context = {}
        context.update({"orders": orders})
        context.update({"title": "My Delivery"})
        return render(request, template_version+"/Views/Order/myDelivery.html", context=context)


@method_decorator(deliveryPerson_only, name='dispatch')
class PendingDelivery(LoginRequiredMixin, View):
    def get(self, request):
        orders = order_models.Orders.objects.filter(~Q(status=3) & ~Q(status=4) & Q(direct_assign__isnull=True) & Q(delivery_by__isnull=True)).order_by('created_at')
        context = {}
        context.update({"orders": orders})
        context.update({"title": "All Pending To Take Delivery."})
        return render(request, template_version+"/Views/Order/pending.html", context=context)
...
class TakeDelivery(LoginRequiredMixin, View):
    def post(self, request):
        ...
        try:
            order = order_models.Orders.objects.get(id=request.POST['orderId'])
        except (Exception, order_models.Orders.DoesNotExist):
            messages.error(request, "No order found.")
            return HttpResponseRedirect(reverse('delivery-order'))
...
class CancelDelivery(LoginRequiredMixin, View):
    def get(self, request):
        orders = order_models.Orders.cancelled_objects.all().order_by('-created_at')
        context = {}
        context.update({"orders": orders})
        context.update({"title": "Cancelled Order"})
        return render(request, template_version+"/Views/Order/cancelled.html", context=context)