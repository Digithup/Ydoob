...
def bill_number_generator(size=6, chars=string.ascii_uppercase + string.digits):
    ...
    number = generate()
    while order_models.Orders.objects.filter(bill_number=number).exists():
        number = generate()
...
def total_pending_orders(user):
    ...
    order = order_models.Orders.objects.filter(Q(district__in=delivery_person.based_on_district.all()) | Q(
        direct_assign=delivery_person) & ~Q(status=3)).order_by('created_at', 'status')
    return order


def total_pending_admin():
    order = order_models.Orders.objects.filter(
...
def my_deliveries(user):
    ...
    delivery_person = get_my_delivery_object(user)
    order = order_models.Orders.objects.filter(Q(direct_assign=delivery_person) | Q(
...
def my_daily_delivery(user):
    delivery_person = get_my_delivery_object(user)
    order = order_models.Orders.objects.filter(Q(delivery_by=delivery_person) & Q(status=3)).extra(
...
def orders_to_be_taken(user):
    ...
    delivery_person = get_my_delivery_object(user)
    order = order_models.Orders.objects.filter(~Q(status=3) & Q(direct_assign__isnull=True) & Q(
...
def cancelled_orders(user):
    delivery_person = get_my_delivery_object(user)
    order = order_models.Orders.cancelled_objects.filter(
...
def cancelled_order_admin():
    order = order_models.Orders.cancelled_objects.all()
    return order
